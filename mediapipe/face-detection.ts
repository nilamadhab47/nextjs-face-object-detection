...
const FaceDetection = (() => {
  ...
  let faceDetectedCallback: any = null;
  ...
  const detectFace = (video: HTMLVideoElement, onFaceDetected: (isFaceDetected: boolean)=> void) => {
    if (faceDetector) {
      try {
        const detection: FaceDetectorResult = faceDetector.detectForVideo(
          video,
          performance.now()
        );
        ...
        if (faceDetectedCallback) {
          faceDetectedCallback(isFaceDetected);
        }
        return detection;
      } catch (error) {
        if (error instanceof Error) {
  ...
  const draw = (
    ...
    sendImageCallback: (base64Image: string) => void // Add this parameter
  ) => {
    if (detections) {
      ...
      detections.forEach((detected: Detection) => {
        if (detected.boundingBox) {
          ...
          const faceRegionX = mirrored
            ? width - box.originX - box.width
            : box.originX;
          const faceRegionY = box.originY;
          const faceRegionWidth = box.width;
          const faceRegionHeight = box.height;

          // Set canvas size
          canvas.width = faceRegionWidth;
          canvas.height = faceRegionHeight;
          console.log(canvas.width, canvas.height);
          // Ensure the canvas has been appended to the document body before trying to access its context
          ...
          // Convert the face region to base64 image
          const base64Image = canvas.toDataURL("image/jpeg");
          sendImageCallback(base64Image);
          // console.log(base64Image)
          // Remove the canvas from the document body after use
          ...
          if (mirrored) {
            ...
            points.push(
              new THREE.Vector2(leftPoint + box.originX, topPoint - box.originY)
            );
            points.push(
              new THREE.Vector2(
