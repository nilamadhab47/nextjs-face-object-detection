    ...
    const detectObject = (
        ...
        if (objectDetector) {
            try {
                ...
                const personDetections = detection.detections.filter(
                    (detected) => detected.categories.some((category) => category.categoryName === "person")
                );
    
                return { detections: personDetections };
            } catch (error) {
                if (error instanceof Error) {
                    console.log(error.message);
                } else {
